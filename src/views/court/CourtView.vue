<template>
  <div class="court-view">
    <!-- 全屏地图 -->
    <div id="court-map" class="court-view__map"></div>

    <!-- 左侧开放城市列表 -->
    <div class="court-view__city-list">
      <div class="city-list__header">
        <span class="city-list__title">开放城市</span>
        <div class="city-list__refresh" :class="{ 'city-list__refresh--loading': cityRefreshing }" @click="refreshCities">
    ...
    <div v-if="!amapMode" class="court-view__search">
      <div class="search-panel" :class="{ 'search-panel--collapsed': searchCollapsed }">
        <div v-show="!searchCollapsed" class="search-panel__body">
          <div class="search-panel__content">
            <a-form layout="inline" :model="searchForm" class="search-form">
              <a-form-item label="名称">
                <a-input v-model:value="searchForm.name" placeholder="球场名称" allowClear class="search-input" @keyup.enter="handleSearch" />
              </a-form-item>
              <a-form-item label="状态">
                <a-select v-model:value="searchForm.status" placeholder="全部" allowClear class="search-input">
              ...
              <a-form-item label="材质">
                <a-select v-model:value="searchForm.material" placeholder="全部" allowClear class="search-input">
              ...
              <a-form-item label="收费">
                <a-select v-model:value="searchForm.isFree" placeholder="全部" allowClear class="search-input search-input--sm">
                  <a-select-option :value="1">免费</a-select-option>
                  <a-select-option :value="0">收费</a-select-option>
                </a-select>
              </a-form-item>
              <a-form-item label="有灯">
                <a-select v-model:value="searchForm.hasLight" placeholder="全部" allowClear class="search-input search-input--sm">
                  <a-select-option :value="1">有灯</a-select-option>
                  <a-select-option :value="0">无灯</a-select-option>
                </a-select>
              </a-form-item>
              <a-form-item label="来源">
                <a-select v-model:value="searchForm.source" placeholder="全部" allowClear class="search-input search-input--sm">
        ...
        <div class="search-panel__footer">
          ...
          <div class="search-panel__toggle" @click="searchCollapsed = !searchCollapsed">
            <DownOutlined v-if="searchCollapsed" />
            <UpOutlined v-else />
            <span class="toggle-text">{{ searchCollapsed ? '展开搜索' : '收起' }}</span>
          </div>
        </div>
      </div>
    </div>
          ...
          <div class="search-panel__content">
            <a-form layout="inline" :model="amapForm" class="search-form">
              <a-form-item label="关键词">
                <a-input v-model:value="amapForm.keywords" placeholder="如：篮球场" allowClear class="search-input" @keyup.enter="handleAmapSearch" />
              </a-form-item>
              <a-form-item label="类型">
                <a-input v-model:value="amapForm.types" placeholder="如：体育场馆" allowClear class="search-input" @keyup.enter="handleAmapSearch" />
              </a-form-item>
              <a-form-item label="区域">
                <a-input v-model:value="amapForm.region" placeholder="如：北京市" allowClear class="search-input" @keyup.enter="handleAmapSearch" />
              </a-form-item>
            </a-form>
            <div class="search-panel__actions">
              <a-button type="primary" size="small" :loading="amapSearching" @click="handleAmapSearch">搜索</a-button>
    ...
    <div class="court-view__float-actions">
      <a-tooltip placement="left" :title="amapMode ? '返回数据管理' : '高德地图搜索'">
        <div
      ...
      <a-tooltip placement="left" title="搜索">
        <div class="float-action-btn" @click="amapMode ? handleAmapSearch() : (searchCollapsed = false, handleSearch())">
          <SearchOutlined />
        </div>
      </a-tooltip>
      <template v-if="!amapMode">
        <a-tooltip placement="left" title="添加球场">
          <div class="float-action-btn float-action-btn--primary" @click="enterAddMode">
            <PlusOutlined />
          </div>
        </a-tooltip>
      </template>
    ...
    <div v-if="addMode" class="court-view__tip">
      <a-alert message="添加模式：点击地图选点" type="info" show-icon closable @close="addMode = false" />
    </div>
    ...
    <div v-show="!amapMode" class="court-view__cards">
      <div ref="cardStripRef" class="card-strip">
        ...
        />
        <div v-if="!loading && courtList.length === 0" class="card-strip__empty">
          暂无球场数据
        </div>
      </div>
    </div>
    ...
    >
      <a-form ref="formRef" :model="form" :rules="rules" layout="vertical">
        ...
        <a-row :gutter="16">
          ...
          <a-col :span="8">
            <a-form-item label="城市编码">
              <a-input v-model:value="form.cityCode" placeholder="自动识别" />
            </a-form-item>
          </a-col>
          <a-col :span="8">
            <a-form-item label="来源">
              <a-select v-model:value="form.source">
                <a-select-option :value="1">官方采集</a-select-option>
                <a-select-option :value="2">用户提交</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
        </a-row>
        ...
        <a-row :gutter="16">
          <a-col :span="12">
            <a-form-item label="经度" name="longitude">
              <a-input-number v-model:value="form.longitude" :precision="6" style="width: 100%" placeholder="点击地图自动填入" />
            </a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="纬度" name="latitude">
              <a-input-number v-model:value="form.latitude" :precision="6" style="width: 100%" placeholder="点击地图自动填入" />
            </a-form-item>
          </a-col>
        </a-row>
        ...
        <a-row :gutter="16">
          ...
          <a-col :span="12">
            <a-form-item label="是否有灯">
              <a-switch v-model:checked="form.hasLight" checked-children="有灯" un-checked-children="无灯" />
            </a-form-item>
          </a-col>
        </a-row>
        <a-row :gutter="16">
          <a-col :span="12">
            <a-form-item label="是否收费">
              <a-switch v-model:checked="form.isFree" checked-children="免费" un-checked-children="收费" />
            </a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="收费描述">
              <a-input v-model:value="form.feeDesc" placeholder="如：10元/人" :disabled="form.isFree" />
            </a-form-item>
          </a-col>
        </a-row>
        <a-row :gutter="16">
          <a-col :span="8">
            <a-form-item label="全场数">
              <a-input-number v-model:value="form.fullCourtCount" :min="0" :max="9" style="width: 100%" />
            </a-form-item>
          </a-col>
          <a-col :span="8">
            <a-form-item label="半场数">
              <a-input-number v-model:value="form.halfCourtCount" :min="0" :max="9" style="width: 100%" />
            </a-form-item>
          </a-col>
          <a-col :span="8">
            <a-form-item label="开放时间">
              <a-input v-model:value="form.openHours" placeholder="06:00-22:00" />
            </a-form-item>
          </a-col>
        </a-row>
        ...
        <a-form-item v-if="form.status === 3" label="驳回原因">
          <a-textarea v-model:value="form.rejectReason" :rows="3" placeholder="请输入驳回原因" />
        </a-form-item>
      </a-form>
...
import { PlusOutlined, DownOutlined, UpOutlined, SearchOutlined, EnvironmentOutlined, DatabaseOutlined, ReloadOutlined } from '@ant-design/icons-vue'
import { message } from 'ant-design-vue'
import AMapLoader from '@amap/amap-jsapi-loader'
import { useUserStore } from '@/store/user'
import { getOpenCityList } from '@/api/city'
import {
  getCourtList, getCourtDetail, addCourt, updateCourt,
  updateCourtStatus, deleteCourt,
} from '@/api/court'
...
const handleCityClick = (city) => {
  ...
  if (map.value) {
    map.value.setZoomAndCenter(12, [Number(city.longitude), Number(city.latitude)], false, 600)
  }
  fetchList()
}
...
const refreshCities = async () => {
  ...
  try {
    ...
    if (userStore.openCourts.length && !activeCityId.value) {
...
const addMarker = (court) => {
  ...
  marker.setContent(`<div style="background:${colorMap[court.status] || '#999'};width:14px;height:14px;border-radius:50%;border:2px solid #fff;box-shadow:0 1px 3px rgba(0,0,0,.3);cursor:pointer;"></div>`)
  ...
  marker.on('click', () => {
    ...
    const html = `
      <div style="padding:14px;min-width:220px;font-size:13px;line-height:1.8;background:rgba(255,255,255,0.92);backdrop-filter:blur(8px);border-radius:10px;box-shadow:0 2px 12px rgba(0,0,0,0.15);position:relative;">
        ...
        <div style="color:#4e5969;">📍 ${c.address || '-'}</div>
        <div style="color:#4e5969;">🏷️ <span style="color:${colorMap[c.status]}">${statusLabelMap[c.status]}</span></div>
        <div style="color:#4e5969;">📋 ${materialLabelMap[c.material] || '-'} · ${c.hasLight ? '有灯' : '无灯'} · ${c.isFree ? '免费' : '收费'}</div>
        <div style="color:#4e5969;">👤 ${c.submitterName || '-'}</div>
        <div style="color:#4e5969;">📅 ${c.createTime || '-'}</div>
        <div style="margin-top:10px;display:flex;gap:8px;">
          <button onclick="window.__courtEdit(${c.id})" style="padding:4px 14px;background:#1677ff;color:#fff;border:none;border-radius:6px;cursor:pointer;font-size:12px;">编辑</button>
          <button onclick="window.__courtDelete(${c.id})" style="padding:4px 14px;background:#ff4d4f;color:#fff;border:none;border-radius:6px;cursor:pointer;font-size:12px;">删除</button>
        </div>
      </div>`
    infoWindow.setContent(html)
    infoWindow.open(map.value, marker.getPosition())
...
window.__courtEdit = (id) => {
  const court = markers.value.find((m) => m.getExtData().id === id)?.getExtData()
...
const searchForm = reactive({
  name: '', status: undefined, material: undefined,
  isFree: undefined, hasLight: undefined, source: undefined,
})
...
const handleReset = () => {
  Object.assign(searchForm, { name: '', status: undefined, material: undefined, isFree: undefined, hasLight: undefined, source: undefined })
  handleSearch()
}
...
const defaultForm = () => ({
  id: undefined, name: '', city: '', cityCode: '', address: '', source: 1,
  longitude: undefined, latitude: undefined,
  material: 0, hasLight: false, isFree: true, feeDesc: '',
  fullCourtCount: 0, halfCourtCount: 0, openHours: '',
  imageUrls: [], status: 0, rejectReason: '',
})
...
const handleEdit = async (record) => {
  ...
  try {
    ...
    if (res?.data) {
      ...
      fileList.value = (res.data.imageUrls || []).map((url, i) => ({
        uid: `-${i}`, name: `photo-${i}`, status: 'done', url,
      }))
      drawerVisible.value = true
    }
  } catch (e) {
...
onMounted(async () => {
  ...
  // 监听 header 高度变化，动态设置 --header-height
